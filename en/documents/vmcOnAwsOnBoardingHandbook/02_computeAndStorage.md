...
With vSAN, it is required to have a default storage policy and it is this policy which is used by VMs that do not have a storage policy explicitly defined on them. The default storage policy for an SDDC is determined by the number of hosts within the SDDC at the time of provisioning. For example, if a 3-node SDDC is deployed, then a RAID1 policy with an FTT of 1 will be used as the default.
